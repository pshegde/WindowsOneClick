...
namespace OneClickApp
{
    ...
    public struct XMLRPCextendReservationResult
    {
        [XmlRpcMissingMapping(MappingAction.Ignore)]
        public string status;
        [XmlRpcMissingMapping(MappingAction.Ignore)]
    ...
    class Request
    {
		...
		public delegate void connectionParametersReceived(String serverIP, String user, String password, Request request);
        ...
        private void makeResv()
        {
            try
			{
				...
				{
                    this.req_id = create_image.requestid;
				}
			    ...
			    if (success)
				{
				    sendConnectionParameters(server_ip, username_reservation, password_reservation, this);
					Thread.Sleep(2000);
				}
        ...
        public void sendConnectionParameters(String serverIP, String user, String password, Request request)
        {
            try
			{
			    if(threadControl.InvokeRequired)
                {
				    object[] pList = { serverIP, user, password, this };
				    threadControl.BeginInvoke(OnConnectionParametersReceived, pList);
                }
            
                else 
                {
				    OnConnectionParametersReceived(serverIP, user, password, this);
                }
			}
		...
		public bool AcceptCertificateNoMatterWhat(object sender,
